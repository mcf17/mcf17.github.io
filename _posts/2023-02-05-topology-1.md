...
Rather we use **Lemma 6** again. A topology $\mathcal O'$ on $X$ is larger than any other $\mathcal O_\alpha$s if and only if $\bigcup_\alpha \mathcal O_\alpha \subseteq \mathcal O'$. Therefore, we can take intersections of all topologies on $X$ that contains $\bigcup_\alpha \mathcal O_\alpha$. 
...
Lastly, we argue that the collection we've constructed in RHS is nonempty. This is easily shown by existence of the discrete topology $\mathcal O_{disc}$ on $X$.
...
- $\sup \mathscr O = \bigcap \left\{ \mathcal O : \mathcal O \text{ is a topology on } X \text{ and } \bigcup \mathscr O \subseteq \mathcal O \right\}$.

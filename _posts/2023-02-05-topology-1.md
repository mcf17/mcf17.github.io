...
**Definition 5**. 집합 $X$에 주어진 두 위상 $\mathcal O_1, \mathcal O_2$에 대해 $\mathcal O_1 \subseteq \mathcal O_2$를 만족할 경우 
...
**Observation 6**. $X$위의 비이산위상을 $\mathcal O_{ind}$, 이산위상을 $\mathcal O_{disc}$, 여유한위상을 $\mathcal O_{cofin}$, 여가산위상을 $\mathcal O_{cocnt}$라 표기하면, 다음이 성립한다. 

1. $\mathcal O_{ind} \subseteq \mathcal O_{cofin} \subseteq \mathcal O_{cocnt} \subseteq \mathcal O_{disc}$.
2. $X$가 원소의 개수가 2개 이상인 유한집합이라면 ($1 < \\| X \\| \leq \infty$) $\mathcal O_{ind} \subsetneq \mathcal O_{cofin} = \mathcal O_{cocnt} = \mathcal O_{disc}$.
...
더 생각해보면 2번에 대한 정답은 1번을 이용하는 수밖에는 없습니다. 일단 모든 $O_\alpha$보다 섬세한 위상은 항상 $\bigcup_\alpha O_\alpha$를 포함한다는 것과 동치입니다. 그 중 가장 거친 위상은 1번을 이용한다면 다음과 같이 표현됩니다. 

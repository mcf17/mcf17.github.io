...
2. If $X$ is finite, $X\setminus U$ is finite for every subset $U$ of $X$. Hence $\mathcal O_{cofin} = \mathcal O_{cocnt}= \mathcal O_{disc}$. Since $X$ has at least 2 elements, cofinite topology contains singletons. Therefore $\mathcal O_{ind} \subsetneq \mathcal O_{cofin}$.
3. Similar to (2).
...
Here we show that the poset $(\mathcal O(X), \subseteq)$ we've defined earlier is indeed a **complete lattice**.

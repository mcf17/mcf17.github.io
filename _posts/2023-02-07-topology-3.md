...
Consider $\N$ equipped with the cofinite topology $\mathcal O_{cofin}$. For $P$ the set of primes, 
- $\mathrm{int}\_\N(P) = \emptyset$, because $\N\setminus P$ is not finite. 
- $\mathrm{cl}\_\N(P) = \N$, because any nonempty open set in $\N$ intersects with $P$ (why?)

Thus $\partial_\N(P) = \N$. But $\partial_\N(\partial_\N(P)) = \partial_\N(\N) = \N \setminus \N = \emptyset $.  

{% endcapture %}

<div class="notice--info">{{ notice-1 | markdownify }}</div>

However, $\partial_X\partial_X(A) = \partial_X\partial_X\partial_X(A)$. This is because $\mathrm{int}\_X(\partial_X\partial_X(A)) = \emptyset$. To see this, if $x$ is an interior point of $\partial_X\partial_X(A)$, then one must have an open set $U$ satisfying
